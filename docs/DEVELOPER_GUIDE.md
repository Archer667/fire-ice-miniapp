# راهنمای دولوپر — والریا : سیزن اول
...
13. [معماریِ محبوبیت و شورش](#۱۳-معماریِ-محبوبیت-و-شورش)
...
| فایل | کارش چیه |
|---|---|
| `main.py` | اپِ FastAPI، میدل‌ورِ CORS، هندلرِ خطایِ سراسری، ثبتِ همهٔ روترها، ساختِ ایندکس‌هایِ Mongo، و حلقه‌های پس‌زمینهٔ سفرها، بازار و بررسیِ شورش |
...
| `db.py` | یک `AsyncIOMotorClient` می‌سازه و هندلِ کالکشن‌ها (`players`, `campaigns`, `rebellions`, `rebellion_checks`, ...) صادر می‌کنه — هیچ ORM یا اسکیمایی در کار نیست، همه‌چیز dict خامه |
| `auth.py` | تابعِ `get_user` (وابستگیِ FastAPI) برایِ احرازِ هویتِ تلگرام، به‌علاوهٔ سه سطحِ ادمین |
| `routers/rebellions.py` | محاسبهٔ محبوبیتِ روزانه، سهمِ غله، تاسِ شورش، رولِ ۲۴ساعته، تنظیمات و داوریِ ادمین |
...
9. **`config.py`** — چرا شراب با ۶۰ شروع می‌شه و تسلیحات با ۲۰: چون تولیدِ پایهٔ شراب صفره و
...
## ۱۳. معماریِ محبوبیت و شورش

این سیستم فقط یک «تاسِ تصادفی» نیست؛ چهار ورودیِ قابل‌ردگیری داره: **مالیات، سهمِ غلهٔ مردم،
ضیافت، و برد/باختِ جنگ**. مالیات و غله موقعِ بررسیِ روزانه اثر می‌ذارن، ضیافت همون لحظه اعمال
می‌شه و نتیجهٔ جنگ وقتی ادمین نتیجهٔ رولِ لشکرکشی رو ثبت می‌کنه.

### دیتا کجا نگه داشته می‌شه؟

- رویِ سندِ بازیکن: `food_ration`، `popularity_history` و `rebellion_last_check`.
- تو کالکشنِ `rebellions`: پروندهٔ کاملِ هر شورش، مهلت، رول و نتیجه.
- تو کالکشنِ `rebellion_checks`: تاسِ روزانهٔ هر بازیکن. ایندکسِ یکتایِ `tg_id + day`
  نمی‌ذاره با ری‌استارتِ سرور یا اجرایِ هم‌زمان، یک روز دوبار تاس انداخته بشه.
- تنظیمات تو `game_settings` با شناسهٔ `rebellion_settings` ذخیره می‌شن؛ پس دیپلویِ تازه
  تنظیماتِ ادمین رو پاک نمی‌کنه.

### جریانِ بررسی

`main.py` تابعِ `evaluate_rebellions()` رو از watcher صدا می‌زنه. خودِ تابع اجرای سنگین رو به
یک‌بار در ساعت محدود می‌کنه و برایِ هر بازیکن هم اثرِ روزانه و تاس رو فقط یک‌بار ثبت می‌کنه:

1. تولید و وضعیتِ بازیکن تازه می‌شه.
2. غله بر اساسِ تعدادِ نفرات و سطحِ سهمِ انتخابی مصرف می‌شه.
3. اثرِ مالیات و سهمِ غله روی محبوبیت میاد؛ اگه غله کافی نباشه، جریمهٔ کمبود هم اضافه می‌شه.
4. بر اساسِ محبوبیت شانس ساخته و تاس انداخته می‌شه.
5. اگه شورش رخ بده، پرونده و مهلت ساخته می‌شه و به بازیکن و همهٔ ادمین‌ها پیام تلگرام می‌ره.

فرمولِ شانس با سه مرز کار می‌کنه: بالاتر از مرزِ امن صفر، پایین‌تر از مرزِ قطعی ۱۰۰٪، و دو
بازهٔ بینشون با پایه و شیبِ جدا. ترتیبِ مرزها همیشه باید
`guaranteed < high_chance < safe` بمونه؛ اعتبارسنجیِ API همین رو اجبار می‌کنه.

### APIها

| مسیر | کاربرد |
|---|---|
| `GET /api/rebellions/status` | وضعیت محبوبیت، سهم غله و شورشِ فعالِ بازیکن |
| `POST /api/rebellions/ration` | تغییرِ سهمِ غله |
| `POST /api/rebellions/{id}/roleplay` | ثبتِ سناریوی مقابله تا قبل از مهلت |
| `GET /api/admin/rebellions` | فهرستِ پرونده‌ها برای ادمین |
| `GET/PUT /api/admin/rebellion-settings` | خواندن/تغییر تنظیمات؛ تغییر فقط برای ادمینِ کامل |
| `POST /api/admin/rebellions/{id}/resolve` | نتیجهٔ روایی و تغییرِ محبوبیت/طلا/غله/نفرات |

داخلِ روتر برای دسترسی ادمین از wrapperهایِ `admin_user` و `full_admin_user` استفاده شده، چون
`get_admin` و `get_full_admin` باید کاربرِ خروجیِ `get_user` رو بگیرن؛ وصل‌کردنِ مستقیمشون به
`Depends` احراز هویت رو درست انجام نمی‌ده.

### اتصال به بقیهٔ بازی

- نتیجهٔ جنگ از مسیرِ ثبتِ نتیجهٔ رول، برد/باخت و مهاجم/مدافع رو مشخص می‌کنه و همون‌جا محبوبیت
  دو طرف تغییر می‌کنه.
- ضیافت از تنظیماتِ همین سیستم هزینه و محبوبیت می‌گیره، نه از عددِ هاردکدِ جدا.
- داشبوردِ بازیکن وضعیت، تغییر سهم و فرمِ رول رو نشون می‌ده؛ تبِ «شورش‌ها»ی پنل ادمین تنظیمات و
  داوری رو. متدهای واقعی و mock هر دو تو `frontend/src/api.js` باید هم‌زمان تغییر کنن.
- پیام بازیکن با `send_system_message` ثبت می‌شه و خبرِ ادمین‌ها با کمک‌تابعِ push بات می‌ره.
- ری‌استارتِ کامل باید هر دو کالکشنِ `rebellions` و `rebellion_checks` رو هم خالی کنه.

**مرز مهمِ طراحی:** حتی اگه نتیجه «پیروزی شورشیان» باشه، سیستم قلعه رو خودکار جابه‌جا نمی‌کنه.
انتقالِ قلعه باید تصمیمِ روشنِ ادمین باشه و از مسیرِ مدیریتِ خاندان‌ها انجام بشه؛ این‌طوری یک
نتیجهٔ اشتباه یا مبهم، مالکیتِ بازیکن رو ناخواسته نابود نمی‌کنه.
