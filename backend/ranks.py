...
async def scored_players() -> list:
    """همهٔ بازیکن‌ها با امتیاز نهایی (پایه + بونوس مقام) — نزولی مرتب‌شده. این تابع
    منبع مشترک هم برای لیدربرد و هم برای مالکیتِ قلعه‌ها روی نقشه است، پس عمداً
    ادمین‌ها را حذف نمی‌کند — فیلترِ «ادمین در لیدربرد نیاید» فقط داخل routers/leaderboard.py
    روی خروجیِ همین تابع اعمال می‌شود، نه اینجا."""
