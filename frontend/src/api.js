...
export const api = {
  ...
  familyDivorce: (id, penalty_gold) => req(`/api/family/marriages/${id}/divorce`,{method:'POST',body:JSON.stringify({penalty_gold})}),
  ...
  adminMarriageControl: (id, action, body) => req(`/api/admin/family/marriages/${id}/${action}`,{method:'POST',body:JSON.stringify(body)}),
  adminMoveChild: (id, body) => req(`/api/admin/family/children/${id}/move`,{method:'POST',body:JSON.stringify(body)}),
  ...
  leaderboardPage: (page, signal) => MOCK ? Promise.resolve({items:M.leaderboard().slice((page-1)*25,page*25), page, pages:Math.max(1,Math.ceil(M.leaderboard().length/25))}) : req('/api/leaderboard?page='+page, { signal }),
  leaderboardPicture: (key, signal) => req('/api/leaderboard/profile-image?key='+encodeURIComponent(key), { signal }),
  ...
  adminRoleplayScoreSettings: () => MOCK ? Promise.resolve({ max_score: 100 }) : req('/api/admin/roleplay-score/settings'),
  adminSaveRoleplayScoreSettings: (maxScore) => MOCK ? Promise.resolve({ max_score: maxScore }) : req('/api/admin/roleplay-score/settings', { method: 'POST', body: JSON.stringify({ max_score: maxScore }) }),
  adminScoreRoleplay: (id, score) => MOCK ? Promise.resolve({ ok: true, score, max_score: 100 }) : req(`/api/admin/roleplay/${id}/score`, { method: 'POST', body: JSON.stringify({ score }) }),
  adminGetPlayerProfile: (tgId) => MOCK ? Promise.resolve({ name: '', backstory: '', profile_image: null }) : req(`/api/admin/players/${tgId}/profile`),
  adminEditPlayerProfile: (tgId, body) => MOCK ? Promise.resolve({ ok: true, ...body }) : req(`/api/admin/players/${tgId}/profile`, { method: 'POST', body: JSON.stringify(body) }),
  ...
  adminNavalLoot: (id, recipient, amounts) => req(`/api/admin/battles/${id}/loot`, {method: 'POST', body: JSON.stringify({recipient_tg_id: recipient, amounts})}),
