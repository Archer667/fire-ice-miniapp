import AdminFamilyControls from '../components/AdminFamilyControls.jsx';
...
const labels = {proposed:'منتظر پاسخ', accepted:'منتظر تأیید مدیریت', active:'پیوند برقرار', rejected:'رد شده', expired:'مهلت تمام شده', ended:'پایان‌یافته', divorced:'فسخ شده'};
...
function MarriageTerms({penalty}) {
  return <div className="family-pact-terms"><p><strong>پیمان کامل بدون هزینهٔ جداگانه</strong></p><p>غرامت فسخ: <strong>{penalty ? fa(penalty)+' طلا' : 'در درخواست مشخص کن'}</strong>؛ فسخ‌کننده باید این مبلغ را کامل به همسرش بپردازد. بدون طلای کافی، ازدواج و پیمان برقرار می‌مانند.</p><p>تا زمان ازدواج، رول خرابکاری علیه همسر ممنوع است. با فسخ، پیمان وابسته و تولدهای آینده متوقف می‌شوند؛ فرزندان فعلی و مسئولیتشان باقی می‌مانند.</p><p className="page-sub">پیمان کامل دوطرفهٔ قبلی به ازدواج متصل و غرامتش با قبول درخواست به‌روز می‌شود. پیمان‌های گروهی مستقل تغییر نمی‌کنند.</p></div>;
}

function Divorce({marriage, busy, act, gold}) {
  const [confirm,setConfirm]=useState(false);
  const penalty=marriage.penalty_gold;
  const insufficient=Number(gold||0)<penalty;
  return <details className="family-divorce"><summary>فسخ ازدواج و پیمان</summary><p>غرامت پرداختی به همسر: <strong>{fa(penalty)} طلا</strong></p><p className="page-sub">پیمان وابسته لغو و تولدهای آینده متوقف می‌شوند؛ فرزندان متولدشده در خاندان مسئول باقی می‌مانند.</p>{insufficient&&<p className="family-danger">طلای کافی برای غرامت نداری؛ فعلاً امکان فسخ نیست.</p>}<label className="family-confirm"><input type="checkbox" checked={confirm} disabled={busy||insufficient||!penalty} onChange={e=>setConfirm(e.target.checked)}/>پرداخت غرامت و فسخ ازدواج و پیمان را تأیید می‌کنم.</label><button className="btn ghost" disabled={busy||!confirm||insufficient||!penalty} onClick={()=>act(()=>api.familyDivorce(marriage.id,penalty))}>پرداخت {fa(penalty)} طلا و فسخ</button></details>;
}
...
export default function Family() {
  ...
  return <section className="family-page">
    ...
    {tab === 'marriage' && <>
      ...
      : <form className="card" onSubmit={e=>{e.preventDefault();act(async()=>{await api.familyPropose(Number(target),requestId,Number(penalty));setRequestId(crypto.randomUUID());});}}><div className="family-heading"><FamilyIcon kind="marriage" /><h2 className="sect">درخواست ازدواج</h2></div><label className="f">طرف پیوند<select required value={target} onChange={e=>setTarget(e.target.value)}><option value="">انتخاب لرد یا لیدی</option>{candidates.map(p=><option key={p.tg_id} value={p.tg_id}>{p.name} · {p.castle}</option>)}</select></label>{!candidates.length && <p className="page-sub">کاراکتر واجد شرایطی برای درخواست در دسترس نیست.</p>}<label className="f">غرامت فسخ ازدواج و پیمان کامل (طلا)<input type="number" min="1" max="1000000000" step="1" required value={penalty} onChange={e=>setPenalty(e.target.value)} placeholder="مبلغ توافق‌شده را بنویس"/></label><MarriageTerms penalty={Number(penalty)}/><p>سهم تو: {fa(s.marriage_gold/2)} طلا و {fa(s.marriage_wine/2)} شراب</p><p className="page-sub">پس از قبول طرف مقابل، مدیریت پیوند را تأیید می‌کند. با رد یا پایان مهلت، سهم‌های رزروشده کامل بازمی‌گردند.</p><button className="btn" disabled={busy || !target || !Number.isInteger(Number(penalty)) || Number(penalty)<1}>ارسال درخواست و رزرو سهم من</button></form>}
