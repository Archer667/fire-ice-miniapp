...
export default function Admin() {
  ...
  const resolveBattle = async (battle) => {
    ...
    try {
      ...
      await api.adminResolveBattle(id, result, roleplayVisibility[id] || 'participants', winner, losses.attacker || {}, losses.defender || {}, losses.attackerEquipment || {}, losses.defenderEquipment || {}, losses.attackers || {}, losses.attackerEquipments || {});
          ...
          <div className="up u2">
            ...
            {battles && battles.map(b => {
              ...
              return <div className="card" key={b.campaign_id} style={{ marginBottom: 12 }}>
                ...
                <div className="notice-guide" style={{ marginTop: 10 }}><strong>زمان نبرد</strong><span>شروع: {b.started_at ? new Date(b.started_at).toLocaleString('fa-IR') : 'نامشخص'}{b.attacker_joins?.length ? `\n${b.attacker_joins.map(j => `${j.player_name}: ${new Date(j.joined_at).toLocaleString('fa-IR')}`).join('\n')}` : ''}</span></div>
                ...
                <label className="f">برنده</label><div className="grid2">{[...(b.attacker_armies || []).map(a => [a.tg_id, a.player_name]),[b.defender_tg_id,b.defender_name]].filter(x => x[0]).filter((x,i,a) => a.findIndex(y => y[0] === x[0]) === i).map(([id,name]) => <button type="button" key={id} className={`rbtn pick ${roleplayWinners[b.campaign_id] === id ? 'sel' : ''}`} onClick={() => setRoleplayWinners(p => ({...p,[b.campaign_id]:id}))}><div className="n">{name}</div></button>)}</div>
